...
export default function OnboardingPage() {
  ...
  const getCurrentStep = () => {
    ...
    if (!onboardingSteps.step4_ads) return 3;
    if (!onboardingSteps.step5_sync) return 4;
    return 5;
  };
  ...
  return (
    <div className="min-h-screen p-4" style={{ background: 'var(--gradient-dark-primary)' }}>
          ...
          <>
            ...
            <Card className="bg-black/20 backdrop-blur-sm border border-white/10 rounded-lg" style={{boxShadow: '0 8px 32px rgba(31, 38, 135, 0.37)'}}>
              <CardContent className="p-8">
                ...
                {currentStep === 3 && <AdAccountsStep onComplete={() => handleStepComplete('step4_ads', 4)} />}
                {currentStep === 4 && <SyncStep operationId={createdOperationId || selectedOperation || ''} onComplete={() => handleStepComplete('step5_sync', 5)} />}
                {currentStep === 5 && (
