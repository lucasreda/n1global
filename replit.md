...
- **FHB Null ID Validation (Oct 21)**: Implemented robust data quality validation for FHB API responses. System now detects and skips orders with null IDs before database insertion, preventing constraint violations. Invalid orders are logged with warnings (including variable_symbol for tracking), counted in `ordersSkipped` metric, and excluded from processing while allowing valid orders to continue without crashes. Critical for production stability when handling external API data quality issues.
- **FHB Automatic Window Splitting (Oct 21)**: Implemented recursive window splitting to bypass FHB API's 99-page pagination limit and capture unlimited order volumes across time periods. When `fetchFHBOrders` detects 99 pages (9,900 orders), `processWindowWithAutoSplit` automatically divides the period in half and processes each sub-window recursively. Continues splitting (30d → 15d → 7.5d → ...) until all sub-windows contain <9,900 orders. Includes termination guard: when a single calendar day exceeds 9,900 orders, processes the first 9,900 and logs overflow warning (prevents infinite recursion). Returns structured results `{orders, complete, error, hitPageLimit}` with comprehensive logging showing split depth and processing statistics. Enables complete historical backfills for high-volume accounts (322k+ orders tested successfully). Single-day overflow is rare but handled gracefully without blocking sync progress.
- **FHB Staging Table Architecture (Oct 21)**: Implemented two-table staging architecture for European Fulfillment Hub integration. FHB Sync Worker pulls ALL orders from FHB API into `fhb_orders` staging table (no filtering). FHB Linking Worker processes unprocessed staging orders and links them to operations by matching `shopifyOrderPrefix`. Updated Initial Sync to 2-year backfill (24 windows × 30 days). Linking worker optimized with operations cache built once per run, continuous loop processing until backlog cleared, and batch size increased to 500 orders. When FHB sync updates existing staging orders, resets `processedToOrders=false` to trigger re-linking with fresh data. Unmatched orders remain unprocessed for future retry when operation prefixes are configured. System follows Shopify-first philosophy where FHB provides complementary tracking/status updates. Multi-tenant ready: multiple FHB accounts feed the same staging table without conflicts.
- **3-Tier FHB Sync Strategy - Incremental Production Ready (Oct 21)**: Implemented incremental 3-tier synchronization strategy for European Fulfillment Hub optimized for high-volume accounts. **Initial Sync** (2-year backfill) runs once per account in 30-day windows, processing up to 9.900 pedidos (99 páginas) por janela. **Deep Sync** (30 days) runs 2x daily at 6h/18h UTC. **Fast Sync** (10 days) every 30min. Worker enforces strict priority (Initial > Deep > Fast) with reentrancy guards (`isInitialRunning`, `isSyncLoopRunning`) to prevent concurrent heavy jobs. `fetchFHBOrders` caps at 99 pages per window and returns `{orders, complete, error}` - pagination limit is normal behavior (not error), only API failures mark incomplete. Windows always advance (no infinite loops), only marks `initialSyncCompleted=true` if no API errors occurred across all windows. Since primary order data comes from Shopify, FHB provides complementary tracking/status updates - high-volume windows (>9.900 orders) update first 9.900, remainder retains Shopify data. Manual triggers respect worker guards with try/finally. Added `initialSyncCompleted`/`initialSyncCompletedAt` to fhb_accounts table, extended fhb_sync_logs.syncType to 'initial'. Status endpoint shows `pendingInitialCount`.
- **Centralized FHB Sync System (Oct 21)**: Implemented admin-level European Fulfillment Hub integration. FHB accounts pull ALL orders into N1's central database, operations filter by configured prefix. Created fhb_sync_logs table for monitoring, added needsSync/lastSyncAt/syncedFromFhb fields to orders table. Admin dashboard at /inside/admin/settings manages FHB accounts with CRUD, connection testing, and manual sync triggers. Worker auto-starts with server and runs independently of operation syncs, optimizing API usage while maintaining data freshness.
- **Fulfillment Provider Security & Routing Fix (Oct 20)**: Resolved critical multi-tenant security issues in European Fulfillment integration. Applied operation-level authorization (operationAccess middleware) to all 7 EF endpoints, preventing cross-tenant warehouse access. Implemented production-safe TLS with env-gating (createHttpsAgent). Fixed provider factory to use createProvider() with await and correct status='active' filter. All endpoints now load operation-specific credentials and validate user access before execution. Eliminated undefined references with dynamic imports. System now correctly routes to operation-specific warehouse (FHB vs N1 Warehouse) based on fulfillment_integrations table lookup.
