...
- **Simplified User Creation Modal (Oct 25)**: Removed operation selection and warehouse integration wizard from user creation modal. User creation now only requires basic information (name, email, password, role). Operations and warehouse accounts must be configured after user creation through the edit modal's dedicated tabs. This simplifies the onboarding flow and aligns with the principle that integrations should be configured post-creation when the user context is fully established.
