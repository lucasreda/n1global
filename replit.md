...
- **Centralized FHB Sync System (Oct 21)**: Implemented admin-level European Fulfillment Hub integration with intelligent two-tier sync strategy. FHB accounts pull ALL orders into N1's central database, operations filter by configured prefix. Deep sync (30 days) runs 2x daily at 6h/18h UTC; Fast sync (10 days) every 30min. Created fhb_sync_logs table for monitoring, added needsSync/lastSyncAt/syncedFromFhb fields to orders table. Admin dashboard at /inside/admin/settings manages FHB accounts with CRUD, connection testing, and manual sync triggers. Worker auto-starts with server and runs independently of operation syncs, optimizing API usage while maintaining data freshness.
