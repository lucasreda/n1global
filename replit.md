...
- **3-Tier FHB Sync Strategy (Oct 21)**: Implemented intelligent 3-tier synchronization strategy for European Fulfillment Hub to guarantee complete order history. **Initial Sync** (1-year backfill) runs once per account in 30-day windows with robust failure tracking - only marks complete if ALL windows succeed, preventing partial data loss. **Deep Sync** (30 days) runs 2x daily at 6h/18h UTC. **Fast Sync** (10 days) every 30min. Worker enforces strict priority (Initial > Deep > Fast) with reentrancy guards to prevent concurrent heavy jobs. Added `initialSyncCompleted`/`initialSyncCompletedAt` to fhb_accounts table, extended fhb_sync_logs.syncType to 'initial'. Admin can trigger initial sync manually via `/api/admin/fhb-sync/trigger` with syncType='initial'. Status endpoint shows `pendingInitialCount` for monitoring.
