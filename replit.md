...
- **European Fulfillment API Date Filtering & URL Bug Fixes (Oct 26)**: Fixed 3 critical bugs in European Fulfillment integration: (1) URL malformation bug where apiUrl without trailing slash caused requests to `https://api.ecomfulfilment.euapi/leads` instead of correct `https://api.ecomfulfilment.eu/api/leads`. Added URL normalization in constructor and updateCredentials to always ensure trailing slash. (2) Double slash bug in login endpoint (`https://api.ecomfulfilment.eu//api/login`) caused 404 errors. Fixed by removing leading slash from endpoint paths since base URL already includes trailing slash. (3) Removed incorrect "No date filtering" limitation - API DOES support date filtering via `date_from` and `date_to` parameters. Updated both sync service and adapter to calculate and pass date ranges (YYYY-MM-DD format) based on sync type days parameter. Initial sync now correctly filters last 90 days instead of fetching all historical data unnecessarily.
- **Automatic Warehouse Account Sync (Oct 26)**: Implemented automatic 3-month initial sync when warehouse accounts are created through admin user creation wizard. System now: (1) Fixed deep copy bug in wizard using structuredClone() to preserve credentials during multi-account setup, (2) Added initialSyncStatus (pending/in_progress/completed/failed) and initialSyncError tracking columns to user_warehouse_accounts table, (3) Created triggerInitialSyncForAccount methods in FHBSyncService and EuropeanFulfillmentSyncService with 90-day lookback, (4) Modified POST /api/user/warehouse-accounts endpoint to fire async background sync immediately after account creation without blocking HTTP response, (5) Enhanced error logging with detailed context in frontend toast notifications. Background sync transitions through pending → in_progress → completed/failed states, ensuring warehouse integrations are operational immediately with complete order history. Sync failures don't block account creation - status is marked as failed for manual intervention while account remains created.
- **Hardcoded API URLs for Warehouse Providers (Oct 25)**: Removed `apiUrl` field from warehouse provider configuration forms. API URLs are now hardcoded internally in sync services: FHB (credentials-based), European Fulfillment (https://api.ecomfulfilment.eu), eLogy (https://api.elogy.io). Users no longer configure API endpoints - they only provide credentials (email, password, and provider-specific fields). This improves security and prevents configuration errors.
